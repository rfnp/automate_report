...
# def append_single_file(filename):
...
def __append_all_files(extension, directory):
    all_filename = [i for i in glob.glob('{directory}/*.{ext}'.format(directory=directory, ext=extension))]

    combined_csv = pd.concat([pd.read_csv(f) for f in all_filename], ignore_index=True)
...
def run_transform(group_by):
    data = __append_all_files('csv', 'data/sales_product_data')
